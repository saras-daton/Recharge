...
{% set result =set_table_name("recharge_charges_tbl_ptrn",'%recharge%charges',"recharge_charges_tbl_exclude_ptrn",'') %}
...
{% for i in result %}
        ...
        {{extract_nested_value("line_items","subscription_id","string")}} as line_items_subscription_id,
    ...
    where {{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('recharge_charges_lookback') }},0) from {{ this }})
        {% endif %}
            qualify dense_rank() over (partition by a.id, line_items_subscription_id, line_items_sku   order by a.{{daton_batch_runtime()}} desc)=1
